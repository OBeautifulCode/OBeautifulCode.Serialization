...
namespace OBeautifulCode.Serialization.Test
{
    ...
    public static class CollectionsWorkDirectlyWithRegistrationOfGenericTypes
    {
        ...
        public static void ElementTypeOfArrayIsOnlyTypeDiscovered()
        {
            ...
            // Act, Assert
            void ThrowIfListsDiffer(DescribedSerializationBase describedSerialization, List<RegisteredKey> deserialized)
            {
                deserialized.Single().Property.Should().Be(expectedList.Single().Property);
            }

            void ThrowIfArraysDiffer(DescribedSerializationBase describedSerialization, RegisteredValue[] deserialized)
            {
                deserialized.Single().Property.Should().Be(expectedArray.Single().Property);
            }

            // Act, Assert
            expectedList.RoundtripSerializeWithCallbackVerification(ThrowIfListsDiffer, bsonConfigType, jsonConfigType);
            expectedArray.RoundtripSerializeWithCallbackVerification(ThrowIfArraysDiffer, bsonConfigType, jsonConfigType);
        }
    }
